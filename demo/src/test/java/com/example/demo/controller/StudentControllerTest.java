...
public class StudentControllerTest {
	...
	/*
	@Test
	public void testUpdateOfStudent() throws Exception{
		
		Student student = new Student(1, "Tushar");
		Mockito.when(studentService.updateStudentById(Mockito.anyInt(), Mockito.any(Student.class))).thenReturn(student);
		RequestBuilder requesBuilder = MockMvcRequestBuilders.put("/student/0").
		...
		MockHttpServletResponse response = result.getResponse();
		assertEquals(HttpStatus.CREATED.value(), response.getStatus());
		assertEquals("http://localhost/student/0", response.getHeader(HttpHeaders.LOCATION));
	}*/
